...
* `Secure Route` can be enabled to activate TLS encryption (Only TLS v1.3 and v1.2 are supported in Rahti). The options are similar than in [Rahti Routes](../usage/kubernetes-concepts.md#route)
...
Then click in Actions > Edit Route. A YAML representation of the route will appear. You can edit it following the example at the [Concepts Route](../usage/kubernetes-concepts.md#route) page. If for example you want to add TLS support (https support), you need to add inside the `spec` section:
...
Some project with dedicated egress IPs will have to request a new [dedicated IP](../configurations/egress-ip.md) in Rahti and update their firewalls accordingly.
...
To learn more about image caching and access control registry in Rahti, refer to the following article: [Using Rahti Integrated Registry](../usage/images/integrated-registry.md)
