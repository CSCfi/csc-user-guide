...
2. In the Openshift web console, in left hand side menu, click on `Software Catalog` under `Home` dropdown list. To create an application, search for _Apache HTTP Server_, click the _Apache HTTP Server_, and select _Instantiate Template_ on the pop-up window on the right.
