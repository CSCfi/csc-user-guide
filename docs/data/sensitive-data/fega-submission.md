...
Submitting data to FEGA is covered by the [free-of-charge](https://research.csc.fi/free-of-charge-use-cases) use cases, but this doesn't include a mandatory backup copy of the data. Backup storage can be bought as an additional service within FEGA, or submitting organisation can agree on other storage methods for the backup copy in the contracts with CSC. More information about the price of FEGA's internal backup service in the [pricing document](./federated-ega-pricing.pdf).
