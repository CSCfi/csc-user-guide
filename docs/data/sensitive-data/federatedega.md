...
You can use FEGA to submit biomedical datasets to be published under controlled access. In SD Apply service, you can apply or approve access to datasets published with FEGA. The sensitive research data submitted to the service remains in Finland, while at the same time, the public descriptive information about the data (metadata) is made internationally available in central [European Genome-phenome Archive](https://ega-archive.org/studies) (EGA). Each dataset must be associated with a Data Access Committee (DAC), which evaluates data access applications and can grant or deny data access for re-use in SD Apply service. If the application is approved, the applicant can directly access and analyse the sensitive data in a secure and private cloud workspace using CSC Sensitive Data Services for research. Submitting data to FEGA requires a separate contract. Choose the correct user guide below:
