...
- [Overview](#overview): key features and limitations
- [General considerations](#general-considerations): what you should know before getting started
...
- :material-check-circle:{ .lg .middle } **Key features**
  { .csc-grid-card-success }
    ...
    - Uploads up to 100 GB are automatically encrypted and decrypted during upload and download via a web browser. For larger files, a programmatic tool (SD-lock/unlock) is available. Automated encryption key management.
...
- :material-close-circle:{ .lg .middle } **Limitations**
  { .csc-grid-card-error }
    ...
    - CSC projects created before 2013 are incompatible with the current user interface and will not be displayed. For assistance, please contact servciedesk@csc.fi (subject: SD Connect) 
    ...
    - In September 2026, SD Connect has been updated to a new version. Files from the previous version are still compatible, but you may encounter issues due to file format changes. 
</div>
