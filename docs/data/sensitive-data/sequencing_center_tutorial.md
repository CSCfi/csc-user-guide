...
It provides free-of-charege storage space for academic research projects at Finnish universities and research institutes. 
Allas can be used for storing any kind of research data during the active working phase of a research project. 
Allas is intended for storing data for an active research project. 
Typically research projects last several years, but note that Allas can’t be used for data archiving. 
You must remove the data from Allas when the research project ends.
Note also that there is no automatic back-up processes in Allas. 
In technical level Allas is very reliable and fault tolerant, 
...
If you are already using Allas service, you can skip this chapter and start from chapter 2.
...
Add Allas service to your project in MyCSC. Only the project manager can add services. 
...
### 2.1 What are keys for

In case of sensitive data, like for example human nucleotide sequence data, 
...
several times and normally it is wise to use the same keys for all data of a project so that 
key management does not get too complicated. 

Instructions for creating a _crypt4gh_ compatible keys can be found [here](sd_connect/#sensitive-data-encryption-and-upload-with-multiple-encryption-keys-less-than-100-gb)
(key generation processes are described in the second steps of graphical user 
...
so that they can then encrypt the data that they send for you. After that, only the owners 
...
its’ password is lost, the data can’t be opened by any means anymore. 
...
One solution is to store the secret key and a text file containing 
the password to CSC Sensitive Data environment using SD Connect interface. 
...
Crypt4GH encryption tool allows encryption with several public keys.  
Data encrypted with this way can be opened with several secure keys. 
If you will utilize sensitive data services of CSC it is handy to use both 
projects' public key and CSC public key in encryption. This way the data can 
be used both in users local environment and in the sensitive data services of CSC.
...
Researcher Tiina Tutkija wants to use Allas to receive and store human sequence 
data that she will use in her new research project called  AniMINE. The project 
...
Now she creates and stores encryption keys for the project. Tiina has cryp4gh-gui 
...
She clicks the upload button again and this time turns **off** **Encrypt files before upload** and then uploads ONLY 
...
(use ssh, PuTTy, or terminal connection from https://puhti.csc.fi ).
...
```text
a-access +rw project_200011 animine_data_import_1
```
