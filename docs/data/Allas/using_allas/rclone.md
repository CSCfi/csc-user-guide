...
with `s3allas:`. Note that you can have both `allas:` and `s3allas` functional in the same time and that they can still use diffrent Allas projects. However, avoid mixing protocols. If an object is loaded using `allas:` do also all operations with `allas:`.  
