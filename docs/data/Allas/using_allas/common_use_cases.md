...
After upload, s3cmd connects therese segments into one large object, but in case of swift based uploads (a-put, rclone , swift) the large file is also stored as several objects. This is done automatically to a bucket that is named by adding extension `_segments` to the original bucket name. For example, if you would use _a-put_ to upload a large file to bucket _123-dataset_ the actual data would be stored to as several pieces into bucket _123-dataset_segments_ and the _123_dataset_ would contain just a front object that contains information what segements make the stored file. Normally users don't need to operate with the _segments_ buckets and objects inside these buckets should not be deleted or modified. Operations perfomed to the front object are automatically reflected to the segments. 
