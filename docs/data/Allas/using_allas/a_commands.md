...
of the data to be uploaded. Data that locates in:
  ...
  - a) $WRKDIR (Taito) or $SCRATCH (Puhti) is uploaded to bucket: _username-projectNumber_-SCRATCH
...
If you wish to use other than the standard bucket, you can define a bucket name with option _-b_ or _--bucket_.

The compressed dataset will be stored as one object. By default, the object name depends on the
...
other *a_commands* and normally it is not displayed to the user.
