...
[getting started with supercomputing guide](../hpc-quick.md).
...
!!! info "Note: Roihu has separate login nodes for CPU and GPU partitions"
    ...
    Hence, there are different login nodes for building and running programs
    on the respective nodes: `roihu-cpu.csc.fi` and `roihu-gpu.csc.fi`. For
    example, connect to the CPU login node using a command-line SSH client like
    this:
