...
For working with EO data there are three main options:

1) **Special EO services**, which provide both data and advanced ready-to-use processing environments. Usually these give better user experience and efficiency, but the services might be limited in computing power, available tools or options for adding own data. 
2) **Commercial clouds** with local copy of open EO data. The data is practically always saved to object-storage and usually can be downloaded from anywhere as independent service. They provide also general computing services where EO data analysis tools need to be installed by the end-user. The most common option for processing in the cloud are virtual machines. These options usually have some fees, mainly for processing. The data download may be free of charge or then have some small costs depending on the amount of data needed.
3) **Own processing environment** - PC, local cluster. All data needs to be downloaded to this system, which requires extra effort. On the other hand it gives more freedom to select used analysis tools. Usually this does not cause any extra costs.

CSC options do not fit well this categorization, rather they have some features from all of these. CSC computing services provide a lot of computing power and storage space, and they are free of charge for Finnish researchers for academic or educational use. 
...
Puhti has also a lot of [pre-installed applications](#what-applications-are-available-on-puhti), so it is an environment ready to use. cPouta virtual machines are similar to commercial cloud offerings, there all set-up is done by end-user.  In both services, it is only possible to install tools that support Linux.

At CSC are available only some Finnish datasets, so often working with CSC comptuing services requires to download EO data from other services. The available local storage in Puhti and cPouta is 1-20 Tb, more space is available from [Allas object storage](../../../data/Allas/index.md).

Using CSC computing services is technically a little bit demanding, requiring basic Linux skills and ability to use some scripting language or command-line tools. Supercomputers have several specific consepts, so it takes a few hours to get started. The new Puhti web interface makes the start considerably easier, providing several graphical tools. It has also Studio and JupyterLab for easy start with R or Python.
...
* Costs, some commercial datasets might be possible to get for free/less for research
* Preprocessing level
    * Raw data
    * Analysis ready data 
    * Mosaics
...
### Some widely used EO instruments
...
|[GEDI](https://gedi.umd.edu/)|25|-|2018->|Yes|
...
A few Finnish EO datasets are available locally at CSC.

* Sentinel and Landsat mosaics of Finland in Puhti
    * Accessing data in Puhti requires CSC user account with a project where Puhti service is enabled. All Puhti users have **read** access to public data stored on Puhti. For using these files, you do not need to copy them, unless you need to modify them.
* Sentinel-2 L2A data of Finland in Allas
    * Allas files are public, so anybody can download them, also from local laptop or from other services.
...
* STAC for CSC data coming in 2023.

So very likely you need to download data from other services. The commercial data is usually available from data provider, the open datasets have several copies in different services.
...
[**USGS EarthExplorer**](https://earthexplorer.usgs.gov/) provides lots of different US related datasets, also worldwide Landsat mission datasets. It requires free registration. Data can be browsed and downloaded via GUI in web interface and bulk download. USGS is the provider of [Landsat Collection 2 data](https://www.usgs.gov/landsat-missions/landsat-data-access).
    
[**NASA Earthdata**](https://search.earthdata.nasa.gov) provides among many others harmonized Landsat 8 and Sentinel-2 dataset. It requires free registration and download is possible via Graphical web interface and bulk download.
...
[**Google Cloud**](https://cloud.google.com) provides some [public EO datasets in Google Cloud Storage](https://cloud.google.com/storage/docs/public-datasets), inlcuding Sentinel-2 L1C and Landsat Collection 1 data and <a href="https://cloud.google.com/storage/docs/public-datasets/landsat" aria-label="Landsat: Collection 1 data on Google cloud storage">Landsat: Collection 1</a>. Data can be downloaded from here for example with [FORCE](../../../apps/force.md).
    
**[Microsoft planetary computer](https://planetarycomputer.microsoft.com)**. It is currently available in preview.
...
[**Sentinelhub**](https://www.sentinel-hub.com/explore/) provides among others worldwide <a href="https://collections.sentinel-hub.com/sentinel-2-l2a/" aria-label="Sentinel 2 on sentinelhub">Sentinel 2</a> and <a href="https://collections.sentinel-hub.com/sentinel-1-grd/" aria-label="Sentinel 1 on sentinelhub">Sentinel 1</a> data. Commercial service.
...
[**GDAL (OGR)**](../../../apps/gdal.md) - Geospatial Data Abstraction Library.  Collection of command-line tools for accessing and transforming geospatial data, relatively fast and require little computational resources. GDAL is included in many other tools for data reading and writing. [GDAL example for Puhti](https://github.com/csc-training/geocomputing/tree/master/gdal)

[**Julia**](../../../apps/julia.md) - Puhti Julia installation does not include any geospatial packages, but they can be installed by the user. [JuliaGeo](https://github.com/JuliaGeo) provides an overview of packages for geospatial data.
...
[**R**](../../../apps/r-env-for-gis.md) - Puhti R installation includes a lot of geospatial packages, includeing serveral useful for EO data processing: terra, CAST, raster, spacetime.
...
One example of the advanced usage of EO data is for machine learning. If you are interested in the topic, you can find a lot of examples from [CSC machine learning with spatial data course materials]( https://github.com/csc-training/GeoML). For practical guidelines, see [CSC machine learning guide](ml-guide.md)
...
**Commercial clouds**: Amazon, Google Cloud and Microsoft Azure, all provide computing services, their data offerings were listed above. Data download costs might be smaller if using virtual machines of the same cloud provider in the same region.
