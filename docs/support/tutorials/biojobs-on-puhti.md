...
- **Shared memory/threaded/OpenMP applications**
    - Can use more than one core, but all cores mut be in the same node 
    ...
    - Usually best to match the number of cores to the number of threads, 
      but check application documentation 
...
sbatch myjobscript
```
If the job was submitted successfully, you should see message:
```text
...
There are other reasons, but they are less common. Consult **servicedesk@csc.fi** if necessary.
...
Most important things to check are CPU efficiency and memory utilization.
...
or if you have to set some application parameter. 
...
Another command to check the status and resource usage of past jobs is **sacct**. It is handy
