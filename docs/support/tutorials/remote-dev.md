...
3. In the now opened file, add a new block like the following and save the file:
   ```text
   ...
   ```
   Adjust the `HostName` to the system you want to connect to. You can freely choose the label after `Host` (`puhti-software-environment`) and this is how it will appear in the remote host selection of the VS Code Remote SSH extension when you make a connection. We recommend giving each combination of system and software environment you work with a recongnizable name (e.g., `puhti-pytorch`). If you prefer to use an SSH key instead of being prompted for your password, you can add `IdentityFile <path_to_your_keyfile>` to the host configuration block shown above (this is mandatory for LUMI).
