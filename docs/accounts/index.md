# Getting access to CSC services

* [Creating a new user account](creating-a-new-user-account.md)
* [Getting student accounts for a course](getting-student-accounts-for-a-course.md)
* [Getting machine-to-machine robot accounts](getting-machine-to-machine-robot-accounts.md)
* [Getting accounts for international collaborators](getting-accounts-for-international-collaborators.md)
* [Getting access to Pouta](get-access-to-pouta.md)

# Managing account

* [Changing the password](changing-the-password.md)
* [Managing user information](managing-user-information.md)

# Managing projects and services

* [Joining a project](joining-a-project.md)
* [Creating a new project](creating-a-new-project.md)
* [Adding members to a project](adding-members-to-a-project.md)
* [Adding service access for a project](adding-service-access-for-a-project.md)

# Managing resources

* [Applying for resources](applying-for-resources.md)
* [Switching between projects](switching-between-projects.md)
* [Monitoring resource usage](monitoring-resource-usage.md)
