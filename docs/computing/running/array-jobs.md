...
In Puhti, you can use the command `sbatch_commandlist` to execute a list of commands as an array job. This command takes as an input a text file. The command list is splitted into several pieces that are executed as an independent sub-task of a array batch job, automatically generated by the `sbatch_commandlist`. Thus one sub task of an array job may process several commands from the commad list.
