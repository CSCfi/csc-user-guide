...
!!! note "Experimental"
    This service is experimental. Tools and their exact outputs may change, do not rely on it in a production environment.
...
The server exposes a single tool, `search_csc_docs`, which takes a search query and returns the closest matching documentation snippets. The agent decides when to call it and what to search for.
