...
3. [Sign the public key in MyCSC and download SSH certificate](#signing-public-key) (required for Roihu only).
    - To connect to Roihu, sign your public key in MyCSC to generate an SSH
      certficate. SSH certificates are like short-lived SSH public keys and
      significantly improve the security of the system. At CSC, each SSH
      certificate is valid for 24 hours, after which a new one must be
      generated by signing the public key in MyCSC again.
