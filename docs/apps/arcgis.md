...
*   ArcGIS desktop tools ArcGIS Pro and ArcMap support only Windows operating system, which makes installing them impossible in CSC's Puhti-rhel7 supercomputer and very difficult in cPouta cloud, because of limited support of Windows in cPouta.
...
*   Puhti-rhel7 has [ArcGIS Python API](https://developers.arcgis.com/python/) in [geoconda](geoconda.md) module, that can be used also for data analysis. [ArcGIS Python API has Apache 2.0 license](https://github.com/Esri/arcgis-python-api/blob/master/LICENSE).
