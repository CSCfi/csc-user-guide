...
In Puhti-rhel7 you can use `pb`(Parallel Blast) command for large sets of query sequences. The pb program splits a large search jobs into several subjobs, that are executed simultaneously (more below).
...
-    as batch jobs with `pb` command in Puhti-rhel7

## Interactive usage in Puhti-rhel7

To use the latest BLAST version in Puhti-rhel7 first give set up command :
```text
module load biokit
```
...
If your query sequence set contains less than 20 sequences then interactive batch job is probably the most effective way top do the search. However, if your query set contains hundreds or thousands of sequences then utilizing the parallel computing capacity of Puhti-rhel7 is more effective. For this kind of massive blast searches you can utilize the `pb` command.
...
```
If your database is big, building the BLAST indexes may require more that 1 GB of memory ( that is the job specific memory limit in Puhti-rhel7 login nodes). In those cases you can subhmit the job from an interactive batch job (with e.g. 8 GB of memory).
