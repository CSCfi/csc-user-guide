...
function updateTocButtonState() {
  ...
  if (isSmallScreen()) {
    ...
    $('.wm-top-tool').toggleClass('hidden', false);
    // $('.wm-top-tool').toggleClass('invisible', shown);
  } else {
    shown = !$('#main-content').hasClass('wm-toc-hidden');
    $('.wm-top-tool').toggleClass('hidden', shown);
  }
}
...
function updateContentHeight() {
  var iframeContainerFluid = $('.container-fluid', $('iframe').get(0).contentWindow.document);
  if (isSmallScreen()) {
    $('.wm-content-pane').height(iframeWindow.document.body.offsetHeight + 20);
    $('.wm-article').attr('scrolling', 'no');
    iframeContainerFluid.toggleClass('smallpage', true);
  } else {
    $('.wm-content-pane').height('');
    $('.wm-article').attr('scrolling', 'auto');
    iframeContainerFluid.toggleClass('smallpage', false);
  }
}
...
function renderPageToc(parentElem, pageUrl, pageToc) {
  ...
  function addItem(tocItem) {
    ul.append($('<li class="wm-toc-li">')
      .append($(`<a class="wm-article-link wm-page-toc-text wm-page-toc-text-${toc_nav_level}">`)
